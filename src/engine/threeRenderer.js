...
const PLAYER_HEIGHT = 52;             // 3D 世界里的球员身高（与原程序化球员一致）
const KIT_COLORS = [0xe63946, 0x3a86ff]; // 红队 / 蓝队
const GK_COLOR = 0xffd60a;            // 门将
...
// 把贴图里的中性亮色（球衣/球袜）染成队伍颜色，保留皮肤、头发与深色鞋子
function makeKitTexture(image, hex) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    if (mx - mn < 26 && lum > 0.45) {
      const shade = Math.min(1, Math.pow(lum, 0.85));
      d[i] = tr * shade;
      d[i + 1] = tg * shade;
      d[i + 2] = tb * shade;
    }
...
function preparePlayerTemplate(gltf) {
  ...
  if (srcImage && baseMat) {
    kitMaterials = [KIT_COLORS[0], KIT_COLORS[1], GK_COLOR].map((hex) => {
      const m = baseMat.clone();
      m.map = makeKitTexture(srcImage, hex);
      m.roughness = 0.72;
      m.metalness = 0.02;
      m.needsUpdate = true;
      return m;
...
function buildCrowd() {
  ...
  const crowd = new THREE.InstancedMesh(box, mat, lowSpec ? 1500 : 2600);
...
export function render3DFrame(snap) {
  ...
  snap.players.forEach((p, i) => {
    ...
    if (m.userData.avatar && m.userData.mixer) {
      ...
      const ts = p.slide ? 0.25 : Math.max(0.32, Math.min(1.9, 0.45 + spd * 0.62 + (p.kick > 0 ? 0.5 : 0)));
      m.userData.action.timeScale = ts;
...
function distForCamera() {
  ...
  if (aspect < 1.2) return 620;   // 竖屏：拉远，保证两队都在画面里
  if (aspect < 1.5) return 520;   // 方屏/平板
  if (aspect < 1.9) return 470;   // 常见横屏手机、桌面
  return 430;                     // 超宽屏手机：人物更大
}
